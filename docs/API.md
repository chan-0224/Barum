...
**샘플 13종은 익명 계정이 만들어질 때 DB에서 채워진다.** 프론트가 이 엔드포인트를 먼저
부를 필요가 없다 — 첫 화면에서 곧바로 루틴(8번)을 요청해도 화장대가 이미 있다.
정렬은 클렌저→토너→세럼→크림→선크림 순으로 고정이다.
...
**AVOID 배지가 뜨면 그 두 제품은 반드시 한쪽이 APPLY, 다른 쪽이 SKIP으로 온다.**
판정 문구가 "오늘은 하나만 쓰세요"이므로 둘 다 빠지면 경고만 뜨고 바를 게 없어지고,
둘 다 들어오면 경고를 무시하는 셈이 된다. 어느 쪽을 남길지는 날씨·피부 상태를 보고
모델이 정하고, 서버가 `routine.parse()`에서 이 관계를 한 번 더 보장한다.
프론트는 배지와 항목을 따로 그리면 되고 별도 분기가 필요 없다.
